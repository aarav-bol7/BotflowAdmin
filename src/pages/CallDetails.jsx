...
// ─── Call source badge (channel + carrier) ───────────────────────────────────
// Shows where the call came from at a glance: WhatsApp voice, a telephony
// carrier (Tata SmartFlo / Vodafone Idea), or the browser voice widget. The
// backend sends `channel` (whatsapp_voice / telephony / voice_ws) and, for
// telephony, `carrier` ("tata" / "vi") on both caller rows and sessions.
// Ported from frontend-chatbot's DynamicADK_CallUsersPanel so both call
// surfaces label sources identically.
const WhatsAppGlyph = () => (
...
const SOURCE_STYLES = {
  whatsapp:  { label: 'WhatsApp',  Icon: WhatsAppGlyph, bg: 'bg-emerald-100 dark:bg-emerald-900/30', text: 'text-emerald-800 dark:text-emerald-400' },
  tata:      { label: 'Tata',      Icon: null,          bg: 'bg-sky-100 dark:bg-sky-900/30',         text: 'text-sky-800 dark:text-sky-400' },
  vi:        { label: 'Vodafone',  Icon: null,          bg: 'bg-rose-100 dark:bg-rose-900/30',       text: 'text-rose-800 dark:text-rose-400' },
  telephony: { label: 'Telephony', Icon: null,          bg: 'bg-indigo-100 dark:bg-indigo-900/30',   text: 'text-indigo-800 dark:text-indigo-400' },
  web:       { label: 'Web',       Icon: null,          bg: 'bg-slate-100 dark:bg-slate-800',        text: 'text-slate-600 dark:text-slate-400' },
};

const callSourceKey = (channel, carrier) => {
  const ch = String(channel || '').toLowerCase();
  if (ch.startsWith('whatsapp')) return 'whatsapp';
  if (ch === 'telephony') {
    const ca = String(carrier || '').toLowerCase();
    if (ca === 'tata') return 'tata';
    if (ca === 'vi') return 'vi';
    return 'telephony';
  }
  if (ch === 'voice_ws' || ch === 'voice') return 'web';
  return '';
};

const CallSourceBadge = ({ channel, carrier }) => {
  const key = callSourceKey(channel, carrier);
  if (!key) return null;
  const s = SOURCE_STYLES[key];
  const Icon = s.Icon;
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${s.bg} ${s.text}`}>
      {Icon ? <Icon /> : <Phone className="w-3 h-3 shrink-0" />} {s.label}
    </span>
  );
};
                  ...
                  <tr key={`${c.caller}_${c.botKey}_${idx}`}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <CallSourceBadge channel={c.channel} carrier={c.carrier} />
                      </div>
                    </td>
                    <td className="px-4 py-3">
        ...
        <>
          <div className="mb-6 bg-white dark:bg-slate-900 rounded-xl border border-slate-200/50 dark:border-slate-800/50 p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <CallSourceBadge channel={selectedCaller.channel} carrier={selectedCaller.carrier} />
            ...
            {selectedCaller.sessions?.map((sess) => (
              <div key={sess.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/50 dark:border-slate-800/50 overflow-hidden">
                ...
                >
                  ...
                  <CallSourceBadge channel={sess.channel} carrier={sess.carrier} />
